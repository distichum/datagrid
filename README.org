#+TITLE: datagrid.el - Manipulate 2d data grids
...
(unless (package-installed-p 'datagrid)
   (package-vc-install "https://github.com/distichum/datagrid.el" nil nil 'datagrid))

(require 'datagrid)
#+end_src

** Manually
Add cgpcallnums.el to your load-path and require.
...
* Examples
datagrid.el includes an example datagrid named =datagrid-example=. The following code will use that but it is small enough that the examples will be trivial.
