...
(defun dg-join-test--mk (&rest cols)
  (datagrid-make :columns (vconcat cols)))

(defsubst dg-join-test--col (dg n)
  (aref (datagrid-columns dg) n))

(defsubst dg-join-test--ncols (dg)
...
(ert-deftest datagrid-join-test-left-basic ()
  (let* ((dg1 (dg-join-test--dg1))
    ...
    (should (= (dg-join-test--ncols r) 4))
    (should (equal (datagrid-pull r 0) ["a" "b" "c"]))
    (should (equal (datagrid-pull r 2) ["ay" "bee" nil]))
    (should (equal (datagrid-pull r 3) [10 20 nil]))))
...
(ert-deftest datagrid-join-test-left-on-cons ()
  (let* ((dg1 (dg-join-test--mk
	       (datagrid-column-make :heading "k1" :data ["a" "b"])
	       (datagrid-column-make :heading "v"  :data [1 2])))
	 (dg2 (dg-join-test--mk
	       (datagrid-column-make :heading "k2" :data ["b" "a"])
	       (datagrid-column-make :heading "w"  :data [20 10])))
	 (r (datagrid-left-join dg1 dg2 :on '("k1" . "k2"))))
    (should (equal (datagrid-pull r 2) [10 20]))))

(ert-deftest datagrid-join-test-left-cols-explicit ()
  (let* ((r (datagrid-left-join (dg-join-test--dg1) (dg-join-test--dg2)
				:on 0 :cols '(2))))
    (should (= (dg-join-test--ncols r) 3))
    (should (equal (datagrid-column-heading (dg-join-test--col r 2)) "score"))))

(ert-deftest datagrid-join-test-left-heading-collision ()
  (let* ((dg1 (dg-join-test--mk
	       (datagrid-column-make :heading "id"  :data ["a"])
	       (datagrid-column-make :heading "val" :data [1])))
	 (dg2 (dg-join-test--mk
	       (datagrid-column-make :heading "id"  :data ["a"])
	       (datagrid-column-make :heading "val" :data [99])))
	 (r (datagrid-left-join dg1 dg2 :on "id")))
    (should (equal (datagrid-column-heading (dg-join-test--col r 2)) "val_2"))))
...
(ert-deftest datagrid-join-test-left-suffix-custom ()
  (let* ((dg1 (dg-join-test--mk
    ...
    (should (equal (datagrid-column-heading (dg-join-test--col r 2)) "val.y"))))

(ert-deftest datagrid-join-test-left-empty-key-no-match ()
  (let* ((dg1 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["a" "" nil])))
	 (dg2 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["a" "" nil])
	       (datagrid-column-make :heading "v"  :data [1 2 3])))
	 (r (datagrid-left-join dg1 dg2 :on 0)))
    (should (equal (datagrid-pull r 1) [1 nil nil]))))

(ert-deftest datagrid-join-test-left-first-duplicate-wins ()
  (let* ((dg1 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["a"])))
	 (dg2 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["a" "a"])
	       (datagrid-column-make :heading "v"  :data [1 2])))
	 (r (datagrid-left-join dg1 dg2 :on 0)))
    (should (equal (datagrid-pull r 1) [1]))))
...
(ert-deftest datagrid-join-test-inner-drops-unmatched ()
  (let* ((r (datagrid-inner-join (dg-join-test--dg1) (dg-join-test--dg2)
				 :on 0)))
    (should (equal (datagrid-pull r 0) ["a" "b"]))
    (should (equal (datagrid-pull r 1) [1 2]))
    (should (equal (datagrid-pull r 2) ["ay" "bee"]))
    (should (equal (datagrid-pull r 3) [10 20]))))

(ert-deftest datagrid-join-test-inner-no-matches ()
  (let* ((dg1 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["x" "y"])
	       (datagrid-column-make :heading "v"  :data [1 2])))
	 (dg2 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["a"])
	       (datagrid-column-make :heading "w"  :data [9])))
	 (r (datagrid-inner-join dg1 dg2 :on 0)))
    (should (= (length (datagrid-pull r 0)) 0))))
...
(ert-deftest datagrid-join-test-full-appends-dg2-only ()
  (let* ((r (datagrid-full-join (dg-join-test--dg1) (dg-join-test--dg2)
				:on 0)))
    (should (equal (datagrid-pull r 0) ["a" "b" "c" "d"]))
    (should (equal (datagrid-pull r 1) [1 2 3 nil]))
    (should (equal (datagrid-pull r 2) ["ay" "bee" nil "dee"]))
    (should (equal (datagrid-pull r 3) [10 20 nil 40]))))

(ert-deftest datagrid-join-test-full-no-extras-equals-left ()
  (let* ((dg1 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["a" "b"])
	       (datagrid-column-make :heading "v"  :data [1 2])))
	 (dg2 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["a" "b"])
    ...
    (should (equal (datagrid-pull full 0) (datagrid-pull left 0)))
    (should (equal (datagrid-pull full 2) (datagrid-pull left 2)))))
...
(ert-deftest datagrid-join-test-full-skips-empty-dg2-keys ()
  (let* ((dg1 (dg-join-test--mk
    ...
    (should (= (length (datagrid-pull r 0)) 1))))
...
(ert-deftest datagrid-join-test-anti-keeps-unmatched ()
  (let* ((r (datagrid-anti-join (dg-join-test--dg1) (dg-join-test--dg2)
				:on 0)))
    (should (equal (datagrid-pull r 0) ["c"]))
    (should (equal (datagrid-pull r 1) [3]))))
...
(ert-deftest datagrid-join-test-anti-keeps-empty-keys ()
  (let* ((dg1 (dg-join-test--mk
    ...
    (should (equal (datagrid-pull r 0) ["" nil]))))
...
(ert-deftest datagrid-join-test-semi-drops-empty-keys ()
  (let* ((dg1 (dg-join-test--mk
    ...
    (should (equal (datagrid-pull r 0) ["a"]))))
...
(ert-deftest datagrid-join-test-type-strict-equal ()
  ...
  (let* ((dg1 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data ["1" "2"])
	       (datagrid-column-make :heading "v"  :data [1 2])))
	 (dg2 (dg-join-test--mk
	       (datagrid-column-make :heading "id" :data [1 2])
	       (datagrid-column-make :heading "w"  :data [10 20])))
	 (r (datagrid-left-join dg1 dg2 :on '(0 . 0))))
    (should (equal (datagrid-pull r 2) [nil nil]))))
