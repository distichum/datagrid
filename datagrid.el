...
The key of the sublists is assumed to be the heading. If EXTEND-UNEVEN
is t, then extend lists with nil so that they are all the same length.
If EXTEND-UNEVEN is nil, then take only the data for a row up to the
minimum row length."
  (interactive)
  (let* ((ralist (reverse alist))
	...
	(setq result
	      (cons (datagrid-column-make
		     :heading (car item)
		     :data (vconcat (seq-take item min-length)))
		    result))))
    (vconcat result)))
